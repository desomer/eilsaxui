...
public interface TKRouterEvent extends JSClass {
	...
	default Object constructor(Object nav) {
		_set(tkAnimation, _new());
	    _set(activityMgr, _new());
		...
		JSCallBack doPushState = let(JSCallBack.class, "doPushState", fct("params","query").__(()->{   
				_if(self.navigo().get("nextenable"));
					...
					_elseif_("this.toString()==",txt(STATE_MENU)); // is next
						...
						_set("$xui.intent.action", txt(ACTION_TOGGLE_MENU));
						_set("$xui.intent.url", txt(URL_MENU));
						_var("intent", "{}");
...
//						.consoleDebug("'intent='", "intent")
					...
					_if(backFromIntent,"!=null && this.toString()==", txt(STATE_ROUTE));
						 // to Prev Route
						_set("$xui.intent.prevActivity", "params.url") ;    //TODO a changer
					    _set("$xui.intent.activity", "$xui.tkrouter.activityMgr.idCurrentActivity");
					    _set("$xui.intent.nextActivityAnim", "backFromIntent.nextActivityAnim");
					    _set("$xui.intent.url", txt("?"));
					    
					    _set("$xui.intent.action", txt(ACTION_PREV_ROUTE));
						__(self.doAction(txt(ACTION_PREV_ROUTE)));
						
					_elseif_("this.toString()==", txt(STATE_ROUTE)); // is next
						// to Next Route
						_set("$xui.intent.prevActivity", "$xui.tkrouter.activityMgr.idCurrentActivity");
					    _set("$xui.intent.activity", "params.url");
					    _set("$xui.intent.url", "toRoute.url");
					    
					    _set("$xui.intent.action", txt(ACTION_NEXT_ROUTE));
					    _var("intent", "{}");
					    __("$.extend(intent, $xui.intent)");
//						.consoleDebug("'intent='", "intent")
